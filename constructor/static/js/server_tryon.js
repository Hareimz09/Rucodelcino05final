(function () {
    ...
    function setImageSource(imageEl, src, isPlaceholder = false) {
        if (!imageEl) return;
        imageEl.src = src || '';
    ...
    window.RucodelTryOn = {
        create(config) {
            ...
            const initialUserPlaceholder = config.initialUserPlaceholder || userPhoto?.dataset.placeholderSrc || userPhoto?.getAttribute('src') || '';
            const initialResultPlaceholder = config.initialResultPlaceholder || aiPhoto?.dataset.placeholderSrc || aiPhoto?.getAttribute('src') || '';
            ...
            function resetResultPreview() {
                setImageSource(aiPhoto, initialResultPlaceholder, true);
                if (downloadEl) {
            ...
            function applyUserImage(dataUrl) {
                ...
                setImageSource(userPhoto, dataUrl, false);
            ...
            if (userImageDataUrl) {
                ...
                setImageSource(userPhoto, initialUserPlaceholder, true);
                resetResultPreview();
            }
            ...
            async function generate() {
                ...
                try {
                    ...
                    setImageSource(aiPhoto, data.result_image, false);
                    if (downloadEl) {
                    ...
                    if (warningEl && Array.isArray(data.warnings) && data.warnings.length) {
                        const visibleWarnings = data.warnings.filter((item) => item !== 'Лицо не найдено автоматически, использована приблизительная посадка.');
                        warningEl.textContent = visibleWarnings.join(' ');
                    } else if (warningEl) {
                    ...
                    let providerLabel = 'AI-примерка готова';
