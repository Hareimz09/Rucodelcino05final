from __future__ import annotations

from decimal import Decimal

from django.conf import settings
from django.db import models
from django.utils import timezone


class CatalogNamedModel(models.Model):
    """Базовый класс для управляемых через админку сущностей каталога."""

    name = models.CharField(max_length=120)
    sort_order = models.PositiveIntegerField(default=100)
    is_active = models.BooleanField(default=True)

    class Meta:
        abstract = True
        ordering = ("sort_order", "name")

    def __str__(self) -> str:  # pragma: no cover - представление для админки
        return self.name


class YarnBrand(CatalogNamedModel):
    slug = models.SlugField(max_length=140, unique=True)
    description = models.TextField(blank=True)

    class Meta(CatalogNamedModel.Meta):
        verbose_name = "Фирма пряжи"
        verbose_name_plural = "Фирмы пряжи"


class YarnColor(CatalogNamedModel):
    brand = models.ForeignKey(
        YarnBrand,
        on_delete=models.CASCADE,
        related_name="colors",
    )
    slug = models.SlugField(max_length=140)
    hex_value = models.CharField(max_length=7, help_text="HEX-цвет вида #AABBCC")
    swatch_asset_path = models.CharField(
        max_length=255,
        blank=True,
        help_text="Путь к статическому изображению палитры, например img/catalog/swatches/graphite.png",
    )

    class Meta(CatalogNamedModel.Meta):
        verbose_name = "Цвет пряжи"
        verbose_name_plural = "Цвета пряжи"
        constraints = [
            models.UniqueConstraint(fields=("brand", "slug"), name="uniq_yarn_color_per_brand")
        ]

    @property
    def full_label(self) -> str:
        return f"{self.name} · {self.brand.name}"


class HatModel(CatalogNamedModel):
    slug = models.SlugField(max_length=140, unique=True)
    preview_asset_path = models.CharField(
        max_length=255,
        help_text="Путь к PNG/WebP/SVG на прозрачном фоне внутри static.",
    )
    description = models.TextField(blank=True)
    render_preset = models.JSONField(default=dict, blank=True)

    class Meta(CatalogNamedModel.Meta):
        verbose_name = "Модель шапки"
        verbose_name_plural = "Модели шапок"


class HatKnitStyle(CatalogNamedModel):
    code = models.SlugField(max_length=80, unique=True)
    description = models.TextField(blank=True)
    texture_density = models.PositiveIntegerField(default=18)
    texture_scale = models.DecimalField(max_digits=4, decimal_places=2, default=Decimal("1.00"))

    class Meta(CatalogNamedModel.Meta):
        verbose_name = "Тип вязки"
        verbose_name_plural = "Типы вязки"


class StoneCatalogItem(CatalogNamedModel):
    slug = models.SlugField(max_length=140, unique=True)
    preview_asset_path = models.CharField(
        max_length=255,
        blank=True,
        help_text="Путь к статическому превью камня внутри static.",
    )
    color_hex = models.CharField(max_length=7, blank=True)
    diameter_mm = models.DecimalField(max_digits=5, decimal_places=1)
    visual_length_mm = models.DecimalField(
        max_digits=6,
        decimal_places=1,
        null=True,
        blank=True,
        help_text="Если элемент визуально занимает больше места, чем диаметр, задайте это значение.",
    )
    material = models.CharField(max_length=120, blank=True)
    shape = models.CharField(max_length=60, default="round")
    description = models.TextField(blank=True)
    metadata = models.JSONField(default=dict, blank=True)

    class Meta(CatalogNamedModel.Meta):
        verbose_name = "Камушек"
        verbose_name_plural = "Камушки"

    @property
    def occupied_length_mm(self) -> Decimal:
        return self.visual_length_mm if self.visual_length_mm is not None else self.diameter_mm


class ClaspType(CatalogNamedModel):
    slug = models.SlugField(max_length=140, unique=True)
    preview_asset_path = models.CharField(
        max_length=255,
        blank=True,
        help_text="Путь к статическому превью застёжки внутри static.",
    )
    visual_length_mm = models.DecimalField(max_digits=6, decimal_places=1, default=Decimal("14.0"))
    material = models.CharField(max_length=120, blank=True)
    description = models.TextField(blank=True)

    class Meta(CatalogNamedModel.Meta):
        verbose_name = "Застёжка"
        verbose_name_plural = "Застёжки"


class MasterInquiry(models.Model):
    """Снимок конфигурации, отправленный мастеру из панели конструктора."""

    CATEGORY_HAT = "hat"
    CATEGORY_JEWELRY = "jewelry"
    CATEGORY_CHOICES = (
        (CATEGORY_HAT, "Шапка"),
        (CATEGORY_JEWELRY, "Украшение"),
    )

    STATUS_NEW = "new"
    STATUS_IN_PROGRESS = "in_progress"
    STATUS_DONE = "done"
    STATUS_CHOICES = (
        (STATUS_NEW, "Новая"),
        (STATUS_IN_PROGRESS, "В работе"),
        (STATUS_DONE, "Закрыта"),
    )

    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name="master_inquiries",
    )
    category = models.CharField(max_length=20, choices=CATEGORY_CHOICES)
    summary = models.TextField(blank=True)
    comment = models.TextField(blank=True)
    selections = models.JSONField(default=dict, blank=True)
    preview_image = models.ImageField(upload_to="master_inquiries/%Y/%m/", blank=True)
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default=STATUS_NEW)
    created_at = models.DateTimeField(auto_now_add=True)

    class Meta:
        ordering = ("-created_at",)
        verbose_name = "Заявка мастеру"
        verbose_name_plural = "Заявки мастеру"

    def __str__(self) -> str:  # pragma: no cover - представление для админки
        category_name = dict(self.CATEGORY_CHOICES).get(self.category, self.category)
        return f"{self.user} • {category_name} • {self.created_at:%d.%m.%Y %H:%M}"

    @property
    def category_label(self) -> str:
        return dict(self.CATEGORY_CHOICES).get(self.category, self.category)


class ChatMessage(models.Model):
    """Сообщение в чате между клиентом и мастером."""

    SENDER_USER = "user"
    SENDER_MASTER = "master"

    SENDER_CHOICES = (
        (SENDER_USER, "Пользователь"),
        (SENDER_MASTER, "Мастер"),
    )

    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name="chat_messages",
    )
    sender = models.CharField(max_length=10, choices=SENDER_CHOICES)
    text = models.TextField(blank=True)
    linked_generation = models.ForeignKey(
        "TryOnGeneration",
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name="chat_messages",
    )
    external_reference_url = models.URLField(blank=True)
    created_at = models.DateTimeField(auto_now_add=True)

    class Meta:
        ordering = ("created_at",)

    def __str__(self) -> str:  # pragma: no cover - представление для админки
        sender_name = dict(self.SENDER_CHOICES).get(self.sender, self.sender)
        if self.linked_generation_id:
            return f"{sender_name}: работа #{self.linked_generation_id}"
        return f"{sender_name}: {self.text[:30]}"

    @property
    def has_reference(self) -> bool:
        return bool(self.linked_generation_id or self.external_reference_url)


class RecoveryRequest(models.Model):
    """Анкета на восстановление доступа, отправленная пользователем."""

    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name="recovery_requests",
    )
    name = models.CharField(max_length=255, blank=True)
    email = models.EmailField()
    details = models.TextField()
    created_at = models.DateTimeField(auto_now_add=True)

    class Meta:
        ordering = ("-created_at",)

    def __str__(self) -> str:  # pragma: no cover - представление для админки
        return f"{self.email}: {self.details[:30]}"


class TryOnGeneration(models.Model):
    """Сохранённый результат AI-примерки пользователя."""

    CATEGORY_HAT = "hat"
    CATEGORY_JEWELRY = "jewelry"
    CATEGORY_CHOICES = (
        (CATEGORY_HAT, "Шапка"),
        (CATEGORY_JEWELRY, "Украшение"),
    )

    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name="tryon_generations",
    )
    category = models.CharField(max_length=20, choices=CATEGORY_CHOICES)
    summary = models.TextField(blank=True)
    selections = models.JSONField(default=dict, blank=True)
    provider = models.CharField(max_length=50, default="server-fallback")
    used_ai = models.BooleanField(default=False)
    warnings_text = models.TextField(blank=True)
    consumed_extra_credit = models.BooleanField(default=False)
    user_image = models.ImageField(upload_to="tryons/originals/%Y/%m/")
    accessory_image = models.ImageField(upload_to="tryons/accessories/%Y/%m/")
    result_image = models.ImageField(upload_to="tryons/results/%Y/%m/")
    is_public_gallery = models.BooleanField(default=False)
    gallery_title = models.CharField(max_length=140, blank=True)
    title_was_edited = models.BooleanField(default=False)
    gallery_description = models.TextField(blank=True)
    created_at = models.DateTimeField(auto_now_add=True)

    class Meta:
        ordering = ("-created_at",)

    def __str__(self) -> str:  # pragma: no cover - представление для админки
        category_name = dict(self.CATEGORY_CHOICES).get(self.category, self.category)
        return f"{self.user} • {category_name} • {self.created_at:%d.%m.%Y %H:%M}"

    @property
    def category_label(self) -> str:
        return dict(self.CATEGORY_CHOICES).get(self.category, self.category)

    @property
    def warnings(self) -> list[str]:
        return [item.strip() for item in self.warnings_text.split("\n") if item.strip()]


class ExtraGenerationPurchase(models.Model):
    """Покупка дополнительных генераций сверх ежемесячного лимита."""

    STATUS_PENDING = "pending"
    STATUS_REVIEW = "review"
    STATUS_PAID = "paid"
    STATUS_CANCELLED = "cancelled"
    STATUS_CHOICES = (
        (STATUS_PENDING, "Ожидает оплаты"),
        (STATUS_REVIEW, "Чек отправлен"),
        (STATUS_PAID, "Одобрено"),
        (STATUS_CANCELLED, "Отклонено"),
    )

    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name="extra_generation_purchases",
    )
    quantity = models.PositiveIntegerField(default=1)
    unit_price_rub = models.PositiveIntegerField(default=25)
    total_price_rub = models.PositiveIntegerField(default=25)
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default=STATUS_PENDING)
    payment_note = models.CharField(max_length=255, blank=True)
    receipt_image = models.ImageField(upload_to="tryons/receipts/%Y/%m/", blank=True)
    receipt_uploaded_at = models.DateTimeField(null=True, blank=True)
    admin_comment = models.CharField(max_length=255, blank=True)
    approved_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.SET_NULL,
        null=True,
        blank=True,
        related_name="approved_extra_generation_purchases",
    )
    approved_at = models.DateTimeField(null=True, blank=True)
    paid_at = models.DateTimeField(null=True, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)

    class Meta:
        ordering = ("-created_at",)

    def __str__(self) -> str:  # pragma: no cover - представление для админки
        return f"{self.user} • {self.quantity} × {self.unit_price_rub} ₽ • {self.get_status_display()}"

    @property
    def status_label(self) -> str:
        return dict(self.STATUS_CHOICES).get(self.status, self.status)

    @property
    def has_receipt(self) -> bool:
        return bool(self.receipt_image)

    def save(self, *args, **kwargs):
        self.quantity = max(int(self.quantity or 1), 1)
        self.unit_price_rub = max(int(self.unit_price_rub or 25), 1)
        self.total_price_rub = self.quantity * self.unit_price_rub

        if self.receipt_image and self.receipt_uploaded_at is None:
            self.receipt_uploaded_at = timezone.now()
        elif not self.receipt_image:
            self.receipt_uploaded_at = None

        if self.status == self.STATUS_PENDING and self.receipt_image:
            self.status = self.STATUS_REVIEW

        if self.status == self.STATUS_PAID:
            if self.paid_at is None:
                self.paid_at = timezone.now()
            if self.approved_at is None:
                self.approved_at = timezone.now()
        else:
            self.paid_at = None
            if self.status != self.STATUS_REVIEW:
                self.approved_at = None
                self.approved_by = None

        super().save(*args, **kwargs)



class GalleryLike(models.Model):
    """Лайк опубликованной работы в общей галерее."""

    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name="gallery_likes",
    )
    generation = models.ForeignKey(
        TryOnGeneration,
        on_delete=models.CASCADE,
        related_name="gallery_likes",
    )
    created_at = models.DateTimeField(auto_now_add=True)

    class Meta:
        ordering = ("-created_at",)
        constraints = [
            models.UniqueConstraint(fields=("user", "generation"), name="uniq_gallery_like_per_user_generation")
        ]

    def __str__(self) -> str:  # pragma: no cover
        return f"{self.user} ♥ {self.generation_id}"


class GalleryComment(models.Model):
    """Комментарий к опубликованной работе."""

    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name="gallery_comments",
    )
    generation = models.ForeignKey(
        TryOnGeneration,
        on_delete=models.CASCADE,
        related_name="gallery_comments",
    )
    parent = models.ForeignKey(
        "self",
        null=True,
        blank=True,
        on_delete=models.CASCADE,
        related_name="replies",
    )
    text = models.TextField()
    created_at = models.DateTimeField(auto_now_add=True)

    class Meta:
        ordering = ("created_at",)

    def __str__(self) -> str:  # pragma: no cover
        return f"{self.user}: {self.text[:40]}"


class GalleryCommentVote(models.Model):
    """Голос пользователя за комментарий."""

    VALUE_UP = 1
    VALUE_DOWN = -1
    VALUE_CHOICES = (
        (VALUE_UP, "Палец вверх"),
        (VALUE_DOWN, "Палец вниз"),
    )

    user = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        on_delete=models.CASCADE,
        related_name="gallery_comment_votes",
    )
    comment = models.ForeignKey(
        GalleryComment,
        on_delete=models.CASCADE,
        related_name="votes",
    )
    value = models.SmallIntegerField(choices=VALUE_CHOICES)
    created_at = models.DateTimeField(auto_now_add=True)

    class Meta:
        ordering = ("-created_at",)
        constraints = [
            models.UniqueConstraint(fields=("user", "comment"), name="uniq_gallery_comment_vote_per_user")
        ]

    def __str__(self) -> str:  # pragma: no cover
        return f"{self.user} → {self.comment_id}: {self.value}"
